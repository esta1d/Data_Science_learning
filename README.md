...
* `Python` с множеством полезных библиотек для работы с данными. Например, для анализа данных использую `Pandas` и `PySpark`. Чтобы визуализировать данные, применяю `Matplotlib`, `Seaborn` и `Plotly` — последний особенно нравится! Для расчёта метрик и прогнозирования задействовал `Scikit-Learn`. Сейчас углубляюсь в CV с изучением `PyCaret` и `TensorFlow`. Всё это освоил на курсе Data Science от SkillFactory.
...
* Самая интерестная работа по моему мнению: [Проект по исследованию поездок на такси New-York](https://github.com/esta1d/SF-DS-practice/blob/main/PROJECT/PROJECT-5/Project-5._Ноутбук-шаблон.ipynb)
