    ...
    LOOP
        ...
        FOR i
            IN (SELECT cur.sobject_Id           sobj,
                  ...
                  FROM DUAL
                --union
                --select 513740 sobj, 'PAL513740', 15 qt, 40600 sobj_prod, to_Date('03/04/2024','dd/mm/yyyy') ex, 2 from dual
                UNION
                SELECT cur.box       sobj,
                       cur.box_name,
                       cur.n01       qt,
        ...
        LOOP
            ...
            sm.smvm_type_id := -6;
            sm.weight := cur.weight * cur.n01; --new corr 19.11.2024
        ...
        INSERT INTO SOBJ_WEIGHT_CONTENT c (c.SOBJECT_ID_PARENT,
                 ...
                 VALUES (
                            ...
                            || '(3103)'
                            || CASE
                                   WHEN LENGTH (TO_CHAR (cur.weight * 1000)) =
                                        3
                                   THEN
                                       '000' || TO_CHAR (cur.weight * 1000)
                                   WHEN LENGTH (TO_CHAR (cur.weight * 1000)) =
        ...
        UPDATE sobj_lnk l
           SET l.QTY = l.qty + i.n01, l.QTY_COVERAGE = l.QTY_COVERAGE + i.n01
                ...
                LOOP
                    UPDATE SOBJ_WEIGHT_CONTENT c
                     ...
                     WHERE     c.SOBJECT_ID_PARENT = REPLACE (cur.v05, 'PAL')
                           ...
                           AND c.SOBJ_PROD_BATCH_ID = cur_wei.sobj_prod_batch_ID
